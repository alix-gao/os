...
os_ret OS_CALLBACK rtw_signal_stat_timer_hdl(RTW_TIMER_HDL_ARGS);
...
#define IEEE80211_RADIOTAP_F_BADFCS 0x40 /* bad FCS */
	...
	if (pattrib->tsfl) {
		...
		mem_cpy(&hdr_buf[rt_len], &tmp_64bit, 8);
		rt_len += 8;
	}
	...
	mem_cpy(&hdr_buf[rt_len], &tmp_16bit, 2);
	rt_len += 2;
...
flog("rtw_signal_stat_timer_hdl\n");
