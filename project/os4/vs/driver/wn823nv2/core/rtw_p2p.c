...
u32 process_probe_req_p2p_ie(struct wifidirect_info *pwdinfo, u8 *pframe, uint len)
{
	...
	u8 *p2pie;
	u32	p2pielen = 0;
	s32 ssid_len=0, rate_cnt = 0;

	p = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_, _SUPPORTEDRATES_IE_, &rate_cnt,
	...
	p = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_, _SSID_IE_, &ssid_len,
			len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_);
...
u32 process_assoc_req_p2p_ie(struct wifidirect_info *pwdinfo, u8 *pframe, uint len, struct sta_info *psta)
{
	...
	while ( p2p_ie )
	{
		...
		if( rtw_get_p2p_attr_content( p2p_ie, p2p_ielen, P2P_ATTR_CAPABILITY, (u8*)&cap_attr, &attr_contentlen) )
		{
		...
		if(rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_INFO, NULL, &attr_contentlen))
		{
			...
			if(pattr_content)
			{
				...
				rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_INFO , pattr_content, &attr_contentlen);
...
u8 process_p2p_provdisc_req(struct wifidirect_info *pwdinfo,  u8 *pframe, uint len )
{
	u8 *frame_body;
	u8 *wpsie;
	u32	wps_ielen = 0, attr_contentlen = 0;
...
u8 process_p2p_group_negotation_req( struct wifidirect_info *pwdinfo, u8 *pframe, uint len )
{
	...
	u32	wps_devicepassword_id_len = 0;
#ifdef CONFIG_WFD
	u8	wfd_ie[ 128 ] = { 0x00 };
	u32	wfd_ielen = 0;
#ifdef CONFIG_TDLS
...
u8 process_p2p_group_negotation_confirm( struct wifidirect_info *pwdinfo, u8 *pframe, uint len )
{
	...
	while ( p2p_ie )	//	Found the P2P IE.
	{
		...
		if ( rtw_get_p2p_attr_content( p2p_ie, p2p_ielen, P2P_ATTR_GROUP_ID, groupid, &attr_contentlen) )
		{
			DBG_871X( "[%s] Ssid = %s, ssidlen = %zu\n", __FUNCTION__, &groupid[ETH_ALEN], str_len(&groupid[ETH_ALEN]) );
			_rtw_memcpy( pwdinfo->groupid_info.go_device_addr, &groupid[0], ETH_ALEN );
			_rtw_memcpy( pwdinfo->groupid_info.ssid, &groupid[6], attr_contentlen - ETH_ALEN );
		}
...
static os_ret OS_CALLBACK reset_ch_sitesurvey_timer_process(os_u32 FunctionContext)
{
	_adapter *adapter = (_adapter *)FunctionContext;
...
static os_ret OS_CALLBACK reset_ch_sitesurvey_timer_process2(os_u32 FunctionContext)
{
	_adapter *adapter = (_adapter *)FunctionContext;
	struct	wifidirect_info		*pwdinfo = &adapter->wdinfo;
...
os_ret OS_CALLBACK restore_p2p_state_timer_process(os_u32 FunctionContext)
{
	_adapter *adapter = (_adapter *)FunctionContext;
	struct	wifidirect_info		*pwdinfo = &adapter->wdinfo;
...
os_ret OS_CALLBACK pre_tx_scan_timer_process(os_u32 FunctionContext)
{
	_adapter 							*adapter = (_adapter *) FunctionContext;
	struct	wifidirect_info				*pwdinfo = &adapter->wdinfo;
	_irqL							irqL;
...
os_ret OS_CALLBACK find_phase_timer_process(os_u32 FunctionContext)
{
	_adapter *adapter = (_adapter *)FunctionContext;
	struct	wifidirect_info		*pwdinfo = &adapter->wdinfo;
...
void rtw_init_wifidirect_timers(_adapter* padapter)
{
	...
	_init_timer( &pwdinfo->find_phase_timer, 0, find_phase_timer_process, padapter );
	_init_timer( &pwdinfo->restore_p2p_state_timer, 0, restore_p2p_state_timer_process, padapter );
	_init_timer( &pwdinfo->pre_tx_scan_timer, 0, pre_tx_scan_timer_process, padapter );
	_init_timer( &pwdinfo->reset_ch_sitesurvey, 0, reset_ch_sitesurvey_timer_process, padapter );
	_init_timer( &pwdinfo->reset_ch_sitesurvey2, 0, reset_ch_sitesurvey_timer_process2, padapter );
#ifdef CONFIG_CONCURRENT_MODE
...
int rtw_p2p_enable(_adapter *padapter, enum P2P_ROLE role)
{
	...
	else if (role == P2P_ROLE_DISABLE)
	{
		...
		if(!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE))
		{
			...
			reset_ch_sitesurvey_timer_process( (os_u32) padapter );
			reset_ch_sitesurvey_timer_process2( (os_u32) padapter );
			#ifdef CONFIG_CONCURRENT_MODE
