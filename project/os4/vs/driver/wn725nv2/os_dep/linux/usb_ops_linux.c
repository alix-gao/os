...
int usbctrl_vendorreq(struct intf_hdl *pintfhdl, u8 request, u16 value, u16 index, void *pdata, u16 len, u8 requesttype)
{
	...
	while(++vendorreq_times<= MAX_USBCTRL_VENDORREQ_TIMES)
	{
        ...
        if ( status == OS_SUCC) {  // Success this control transfer.
            status = len;
#endif
...
unsigned int ffaddr2pipehdl(struct dvobj_priv *pdvobj, u32 addr)
{
	...
	if (addr == RECV_BULK_IN_ADDR) {
		...
		// alic pipe=usb_rcvbulkpipe(pusbd, pdvobj->RtInPipe[1]);
...
static void usb_bulkout_zero_complete(struct zero_bulkout_context *pcontext)
{
	// alic struct zero_bulkout_context *pcontext = (struct zero_bulkout_context *)purb->context;
	...
	if(pcontext)
	{
		...
		if(pcontext->purb /* alic && (pcontext->purb==purb) */)
		{
...
#else
    status = send_usb_bulk_data(pusbd, pipe, pbuf, len);
...
static void usb_write_port_complete(struct xmit_buf *pxmitbuf, int status)
{
	_irqL irqL;
	int i;
	// alic struct xmit_buf *pxmitbuf = (struct xmit_buf *)purb->context;
...
_func_enter_;
    ...
    flog("(%d %d) write msg %d\n", addr, pipe, cnt);
    for (status = 0; status < cnt; status++) {
        flog("%x ", pxmitframe->buf_addr[status]);
    } flog("\n");
