...
#define _rtw_malloc(sz)			kmalloc(sz) //u8*	_rtw_malloc(u32 sz);
...
#endif /* CONFIG_USE_VMALLOC */
static inline u8* _rtw_zmalloc00(u32 sz)
{
	u8 	*pbuf = kmalloc(sz);
...
#define rtw_malloc(sz)			kmalloc(sz) //_rtw_malloc((sz))
#define rtw_zmalloc(sz)			_rtw_zmalloc00((sz))
...
extern void rtw_init_timer(_timer *ptimer, void *padapter, TIMER_FUNC_PTR pfunc);


__inline static unsigned char _cancel_timer_ex(_timer *ptimer)
{
#ifdef PLATFORM_LINUX
    if ((u32) trace_timer == (u32) ptimer) {
        flog("cancel trace timer\n");
    }
