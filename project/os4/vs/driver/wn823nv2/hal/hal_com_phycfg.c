	...
	)
{
	...
	DBG_871X("%s ChannelPlan:0x%x,Regulation(2_4G/5G):0x%x,0x%x\n",
	...
	)
{
	...
	for (BufOfLines = GetLineFromBuffer(ptmp); BufOfLines != NULL; BufOfLines = GetLineFromBuffer(ptmp))
	{
		...
		for ( j = 0; j < str_len(BufOfLines); ++j )
		{
		...
		if (str_len(BufOfLines) != 0)
			i++;
		BufOfLines[str_len(BufOfLines)] = '\n';
	}
}
...
)
{
	#define STR_EQUAL_5G(_band, _path, _sign, _rate, _chnl) \
		((str_cmp(Band, _band) == 0) && (str_cmp(Path, _path) == 0) && (str_cmp(Sign, _sign) == 0) &&\
		(str_cmp(Rate, _rate) == 0) && (str_cmp(Channel, _chnl) == 0)\
	)
	#define STR_EQUAL_2G(_band, _path, _sign, _rate) \
		((str_cmp(Band, _band) == 0) && (str_cmp(Path, _path) == 0) && (str_cmp(Sign, _sign) == 0) &&\
		(str_cmp(Rate, _rate) == 0)\
	)
	...
	if(rtStatus == _SUCCESS)
	{
		...
		for (szLine = GetLineFromBuffer(ptmp); szLine != NULL; szLine = GetLineFromBuffer(ptmp))
		{
			if ( ! IsCommentString(szLine) )
			{
				...
				while ( szLine[i] != '{' && i < str_len(szLine))
					i++;
