...
os_ret OS_CALLBACK pwr_state_check_handler(RTW_TIMER_HDL_ARGS);
os_ret OS_CALLBACK pwr_state_check_handler(RTW_TIMER_HDL_ARGS)
{
	_adapter *padapter = (_adapter *)FunctionContext;
flog("pwr_state_check_handler\n");
	rtw_ps_cmd(padapter);
}
...
_func_enter_;
	...
	if(ps_mode == PS_MODE_ACTIVE)
	{
			...
			)
		{
			...
			if (pwrpriv->lps_leave_cnts < UINT32_MAX)
	...
	else
	{
			...
			)
		{
			...
			if (pwrpriv->lps_enter_cnts < UINT32_MAX)
...
_func_enter_;
	...
	DBG_871X("+LeisurePSLeave %s %d %d\n", msg, pwrpriv->bLeisurePs, pwrpriv->pwr_mode);
...
_func_enter_;
	...
	flog("pwrinfo: %d %d\n", padapter->registrypriv.mp_mode, padapter->registrypriv.power_mgnt);
