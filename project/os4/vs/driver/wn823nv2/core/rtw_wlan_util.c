...
int rtw_check_bcn_info(ADAPTER *Adapter, u8 *pframe, u32 packet_len)
{
		...
		_rtw_memcmp(&recv_beacon, &pmlmepriv->new_beacon_keys, sizeof(recv_beacon)) == _FALSE)
	{
		...
		mem_cpy(&pmlmepriv->new_beacon_keys, &recv_beacon, sizeof(recv_beacon));
	...
	if (pmlmepriv->new_beacon_cnts >= new_bcn_max)
	{
		...
		mem_cpy(&pmlmepriv->cur_beacon_keys, &recv_beacon, sizeof(recv_beacon));
		pmlmepriv->new_beacon_cnts = 0;
	}
	...
	while (*bytes_left > 0) {
		...
		*bytes_left -= 1;
		...
		if (str[0] == 0) {
			/* Broadcast SSID */
			ssid[idx].SSID_len = 0;
			mem_set((char*)ssid[idx].SSID, 0x0, WLAN_SSID_MAXLEN);
			*bytes_left -= 1;
			str += 1;
			...
			*bytes_left -= 1;
			...
			mem_cpy((char*)ssid[idx].SSID, str, ssid[idx].SSID_len);
