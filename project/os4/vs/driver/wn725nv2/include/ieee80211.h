...
u8 *rtw_set_fixed_ie(unsigned char *pbuf, unsigned int len, unsigned char *source, u32 *frlen);
u8 *rtw_set_ie(u8 *pbuf, sint index, uint len, u8 *source, u32 *frlen);
...
u8 *rtw_get_ie(u8*pbuf, sint index, s32 *len, sint limit);
u8 *rtw_get_ie_ex(u8 *in_ie, uint in_len, u8 eid, u8 *oui, u8 oui_len, u8 *ie, u32 *ielen);
int rtw_ies_remove_ie(u8 *ies, u32 *ies_len, uint offset, u8 eid, u8 *oui, u8 oui_len);
...
u8 rtw_is_wps_ie(u8 *ie_ptr, u32 *wps_ielen);
u8 *rtw_get_wps_ie_from_scan_queue(u8 *in_ie, uint in_len, u8 *wps_ie, u32 *wps_ielen, u8 frame_type);
...
int rtw_get_wfd_ie_from_scan_queue(u8 *in_ie, int in_len, u8 *p2p_ie, u32 *p2p_ielen, u8 frame_type);
