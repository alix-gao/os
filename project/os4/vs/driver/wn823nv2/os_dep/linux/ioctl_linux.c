...
os_void wireless_send_event(struct net_device *dev, unsigned int cmd, union iwreq_data *wrqu, const char *extra)
{
    UNDEFINED_FUNCTION;
}
...
static void indicate_wx_custom_event(_adapter *padapter, char *msg)
{
	...
	if (str_len(msg) > IW_CUSTOM_MAX) {
		DBG_871X("%s str_len(msg):%zu > IW_CUSTOM_MAX:%u\n", __FUNCTION__ , str_len(msg), IW_CUSTOM_MAX);
		return;
	}
	...
	_rtw_memcpy(buff, msg, str_len(msg));

	_rtw_memset(&wrqu,0,sizeof(wrqu));
	wrqu.data.length = str_len(msg);
	...
	if(pbuf){
		...
		if (pnetwork->network.Reserved[0] != 2) // Probe Request
		{
			...
			if (wpa_len > 0){
				...
				iwe->cmd = IWEVCUSTOM;
				iwe->u.data.length = str_len(pbuf);
				...
				char *start, char *stop,struct iw_event *iwe)
{
	...
	if (pnetwork->network.Reserved[0] != 2) // Probe Request
	{
		...
		if (wapi_len > 0)
		{
			...
			_rtw_memset(iwe, 0, sizeof(*iwe));
			iwe->cmd = IWEVCUSTOM;
			iwe->u.data.length = str_len(buf_wapi);
...
*/
	...
	if (wrqu->essid.flags && wrqu->essid.length)
	{
		...
		//	=========================================
		//	wrq.u.essid.length = str_len(essid) + 1;
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	DBG_871X( "[%s] ssid = %s, len = %zu\n", __FUNCTION__, extra, str_len( extra ) );
	_rtw_memcpy( pwdinfo->nego_ssid, extra, str_len( extra ) );
	pwdinfo->nego_ssidlen = str_len( extra );
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	wrqu->data.length = str_len( extra );
										...
										union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	wrqu->data.length = str_len(attr_content_str);
										 ...
										 union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	u8 blnMatch = 0;
	u8 *p2pie;
	u32 p2pielen = 0, attr_contentlen = 0;
	...
	wrqu->data.length = str_len(go_devadd_str);
								   ...
								   union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	wrqu->data.length = str_len(dev_type_str);
								   ...
								   union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	wrqu->data.length = str_len(dev_name_str);
											...
											union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	wrqu->data.length = str_len(inv_proc_str);
	...
	union iwreq_data *wrqu, char *extra)
{
	...
	if ((36 == str_len(extra)) && (uuid_str2bin(extra, pwdinfo->uuid) == 0))
	{
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	if( 0 == str_cmp(rereg_priv->old_ifname, new_ifname) ) {
		return ret;
	}
				...
				char *extra)
{
	...
	if (copy_to_user(dwrq->pointer, ext, min(dwrq->length, (u16)(str_len(ext)+1)) ) )
		ret = -EFAULT;

	#ifdef CONFIG_DEBUG_RTW_WX_SET_PRIV
	DBG_871X("%s: %s req=%s rep=%s dwrq->length=%d, str_len(ext)+1=%d\n", __FUNCTION__,
		dev->name, ext_dbg ,ext, dwrq->length, (u16)(str_len(ext)+1));
	#endif
			...
			union iwreq_data *wdata, char *extra)
{
	...
	if(str_cmp(tmp[0], "status") == 0){
		sprintf(extra, "Load File efuse=%s,Load File MAC=%s",(pEEPROM->bloadfile_fail_flag? "FAIL" : "OK"),(pEEPROM->bloadmac_fail_flag? "FAIL" : "OK"));

		  goto exit;
	}
	else if (str_cmp(tmp[0], "drvmap") == 0)
	{
...
//		DBG_871X("\n");
	...
	else if (str_cmp(tmp[0], "realmap") == 0)
	{
		mapLen = EFUSE_MAP_SIZE;
...
//		DBG_871X("}\n");
	...
	else if (str_cmp(tmp[0], "realraw") == 0)
	{
		addr = 0;
		...
		_rtw_memset(extra,'\0',str_len(extra));
		//		DBG_871X("%s: realraw={\n", __FUNCTION__);
				sprintf(extra, "\n0x00\t");
	...
	else if (str_cmp(tmp[0], "realrawb") == 0)
	{
		addr = 0;
		mapLen = EFUSE_MAX_SIZE;
		DBG_871X("EFUSE_MAX_SIZE =%d   \n",EFUSE_MAX_SIZE);
...
//		DBG_871X("}\n");
	...
	else if (str_cmp(tmp[0], "vidpid") == 0)
	{
		#ifdef CONFIG_RTL8192C
		addr = EEPROM_VID_92C;
		#endif // CONFIG_RTL8192C
...
//		DBG_871X("}\n");
	...
	else if (str_cmp(tmp[0], "ableraw") == 0)
	{
		efuse_GetCurrentSize(padapter,&raw_cursize);
		raw_maxsize = efuse_GetMaxSize(padapter);
		sprintf(extra, "[available raw size]= %d bytes", raw_maxsize-raw_cursize);
	}
	else if (str_cmp(tmp[0], "btfmap") == 0)
	{
...
//		DBG_871X("\n");
	...
	else if (str_cmp(tmp[0],"btbmap") == 0)
	{
		BTEfuse_PowerSwitch(padapter,1,_TRUE);
...
//		DBG_871X("\n");
	...
	else if (str_cmp(tmp[0],"btrmap") == 0)
	{
		if ((tmp[1]==NULL) || (tmp[2]==NULL))
		{
			err = -EINVAL;
...
//		DBG_871X("}\n");
	...
	else if (str_cmp(tmp[0], "btffake") == 0)
	{
//		DBG_871X("OFFSET\tVALUE(hex)\n");
		sprintf(extra, "\n");
		for (i=0; i<512; i+=16)
...
//		DBG_871X("\n");
	...
	else if (str_cmp(tmp[0],"btbfake") == 0)
	{
//		DBG_871X("OFFSET\tVALUE(hex)\n");
		sprintf(extra, "\n");
		for (i=512; i<1024; i+=16)
...
//		DBG_871X("\n");
	...
	else if (str_cmp(tmp[0],"wlrfkmap")== 0)
	{
//		DBG_871X("OFFSET\tVALUE(hex)\n");
		sprintf(extra, "\n");
		for (i=0; i<EFUSE_MAP_SIZE; i+=16)
...
//		DBG_871X("\n");
	...
	else if (str_cmp(tmp[0],"wlrfkrmap")== 0)
	{
		if ((tmp[1]==NULL) || (tmp[2]==NULL))
				{
					DBG_871X("%s: rmap Fail!! Parameters error!\n", __FUNCTION__);
	...
	else if (str_cmp(tmp[0],"btrfkrmap")== 0)
	{
		if ((tmp[1]==NULL) || (tmp[2]==NULL))
				{
					DBG_871X("%s: rmap Fail!! Parameters error!\n", __FUNCTION__);
			...
			union iwreq_data *wdata, char *extra)
{
	...
	else if (str_cmp(tmp[0], "vidpid") == 0)
	{
        ...
        else if (str_cmp(tmp[0], "wldumpfake") == 0)
	{
	...
	else if (str_cmp(tmp[0], "btwmap") == 0)
	{
	...
	else if (str_cmp(tmp[0], "btwfake") == 0)
	{
		if ((tmp[1]==NULL) || (tmp[2]==NULL))
		{
			err = -EINVAL;
	...
	else if (str_cmp(tmp[0], "btdumpfake") == 0)
	{
	...
	else if (str_cmp(tmp[0], "btfk2map") == 0)
	{
	...
	else if (str_cmp(tmp[0], "wlfk2map") == 0)
	{
		EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI, TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, (PVOID)&max_available_size, _FALSE);
		if (max_available_size < 1)
		{
	...
	else if (str_cmp(tmp[0], "wlwfake") == 0)
	{
		if ((tmp[1]==NULL) || (tmp[2]==NULL))
		{
			err = -EINVAL;
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	if(qAutoLoad==0)
	{
		...
		wrqu->data.length = str_len(extra) + 1;
	return 0;
}
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	if(strncmp(extra, "off",3) ==0 && str_len(extra)<4)
	{
	...
	if(strncmp(extra, "on",2) ==0 && str_len(extra)<3)
	{
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	if (copy_from_user(extra, wrqu->data.pointer, wrqu->data.length))
			return -EFAULT;
	if(str_len(extra)<1) return -EFAULT;
...
#ifdef CONFIG_BT_COEXIST
	if (str_cmp(pch, "bton") == 0)
	{
		rtw_btcoex_SetManualControl(padapter, _FALSE);
	}
	else if (str_cmp(pch, "btoff") == 0)
	{
		rtw_btcoex_SetManualControl(padapter, _TRUE);
	}
	else if (str_cmp(pch, "h2c") == 0)
	{
		u8 param[8];
		u8 count = 0;
		u32 tmp;
...
static int _rtw_ioctl_wext_private(struct net_device *dev, union iwreq_data *wrq_data)
{
	...
	k = -1;
	while((++k < num_priv_args) && str_cmp(priv_args[k].name, cmdname));
