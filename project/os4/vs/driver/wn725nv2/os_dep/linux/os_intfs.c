...
struct net_device *rtw_init_netdev(_adapter *old_padapter)
{
	...
	if (!pnetdev)
		return NULL;
    pnetdev->br_port = OS_NULL;
	pnetdev->priv = OS_NULL;

	if(old_padapter != NULL) {
	    pnetdev->priv = old_padapter;
	} else {
	    cassert(FALSE);
	}
...
u32 rtw_start_drv_threads(_adapter *padapter)
{
	...
	if(padapter->adapter_type == PRIMARY_ADAPTER){
#endif
	padapter->xmitThread = create_task("RTW_XMIT_THREAD", rtw_xmit_entry_point, WN725N_XMIT_TASK_PRIO, (os_u32) padapter,0,0,0,0,0,0);//kthread_run(rtw_cmd_thread, padapter, "RTW_CMD_THREAD");
...
#endif //CONFIG_CONCURRENT_MODE
	{
		padapter->cmdThread = padapter->cmdThread = create_task("RTW_CMD_THREAD", rtw_cmd_entry_point, WN725N_CMD_TASK_PRIO, (os_u32) padapter,0,0,0,0,0,0);//kthread_run(rtw_cmd_thread, padapter, "RTW_CMD_THREAD");
