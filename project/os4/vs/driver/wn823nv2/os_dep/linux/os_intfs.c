...
uint loadparam(PADAPTER padapter);
...
#else
LOCALC os_ret OS_CALLBACK rtw_cmd_entry_point(os_u32 arg1, os_u32 arg2, os_u32 arg3, os_u32 arg4, os_u32 arg5, os_u32 arg6, os_u32 arg7)
{
    _adapter *padapter;

    padapter = (_adapter *) arg1;
    rtw_cmd_thread(padapter);
}
LOCALC os_ret OS_CALLBACK rtw_xmit_entry_point(os_u32 arg1, os_u32 arg2, os_u32 arg3, os_u32 arg4, os_u32 arg5, os_u32 arg6, os_u32 arg7)
{
    _adapter *padapter;

    padapter = (_adapter *) arg1;
    rtw_xmit_thread(padapter);
}
u32 rtw_start_drv_threads(_adapter *padapter)
{
	u32 _status = _SUCCESS;
	...
	padapter->xmitThread = create_task("RTW_XMIT_THREAD", rtw_xmit_entry_point, RTW_DRIVER_TX_PRI, (os_u32) padapter,0,0,0,0,0,0);//kthread_run(rtw_cmd_thread, padapter, "RTW_CMD_THREAD");
    if(OS_NULL == padapter->xmitThread)
	_status = _FAIL;
#if defined(CONFIG_SDIO_HCI) && defined(CONFIG_CONCURRENT_MODE)
	}
#endif		// CONFIG_SDIO_HCI+CONFIG_CONCURRENT_MODE
...
#endif //CONFIG_CONCURRENT_MODE
	{
		padapter->cmdThread = create_task("RTW_CMD_THREAD", rtw_cmd_entry_point, RTW_DRIVER_CMD_PRI, (os_u32) padapter,0,0,0,0,0,0);//kthread_run(rtw_cmd_thread, padapter, "RTW_CMD_THREAD");
	        if(OS_NULL == padapter->cmdThread)
			_status = _FAIL;
		else
			_rtw_down_sema(&padapter->cmdpriv.terminate_cmdthread_sema); //wait for cmd_thread to run
	}
...
_func_enter_;
	...
	// We don't need to mem_set padapter->XXX to zero, because adapter is allocated by rtw_zvmalloc().
...
{
// alic	struct net_device *pnetdev = (struct net_device*)padapter->pnetdev;
...
void netdev_br_init(struct net_device *netdev)
{
	...
	//if(check_fwstate(pmlmepriv, WIFI_STATION_STATE|WIFI_ADHOC_STATE) == _TRUE)
	{
...
#endif  // (LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 35))
		{
			...
			if (br_netdev) {
				mem_cpy(adapter->br_mac, br_netdev->dev_addr, ETH_ALEN);
				dev_put(br_netdev);
			} else
...
{
	int ret = _SUCCESS;
// alic	struct net_device *pnetdev = padapter->pnetdev;
...
#endif
	DBG_871X("%s, MAC Address (if%d) = " MAC_FMT "\n", __FUNCTION__, (padapter->iface_id+1), MAC_ARG(adapter_mac_addr(padapter)));
...
int pm_netdev_open(_adapter *padapter,u8 bnormal)
{
	int status = 0;
	...
	{
		_enter_critical_mutex(&(adapter_to_dvobj(padapter)->hw_init_mutex), NULL);
// alic		status = _netdev_open(pnetdev);
		_exit_critical_mutex(&(adapter_to_dvobj(padapter)->hw_init_mutex), NULL);
	}
#ifdef CONFIG_IPS
	else
...
static int route_dump(u32 *gw_addr ,int* gw_index)
{
	...
	mem_set(&nladdr, 0, sizeof(nladdr));
...
restart:
	...
	for (;;)
	{
		...
		while (NLMSG_OK(h, err))
		{
			...
			mem_set(&rt_info, 0, sizeof(struct route_info));
...
int rtw_suspend_common(_adapter *padapter)
{
	...
	DBG_871X("==> %s (%s:%d)\n",__FUNCTION__, 0,0);//current->comm, current->pid);
...
{
// alic	struct net_device *pnetdev;
	#ifdef CONFIG_CONCURRENT_MODE
	struct net_device *pbuddy_netdev;
	#endif
...
// alic	pnetdev = padapter->pnetdev;
	...
	if(pm_netdev_open(padapter,_TRUE) != 0) {
		ret = -1;
		pdbgpriv->dbg_resume_error_cnt++;
		goto exit;
	}

// alic	netif_device_attach(pnetdev);
// alic	netif_carrier_on(pnetdev);

	#ifdef CONFIG_CONCURRENT_MODE
	if(rtw_buddy_adapter_up(padapter)){
// alic		pbuddy_netdev = padapter->pbuddy_adapter->pnetdev;

// alic		netif_device_attach(pbuddy_netdev);
// alic		netif_carrier_on(pbuddy_netdev);
	}
	#endif
	...
	if( padapter->pid[1]!=0) {
		DBG_871X("pid[1]:%d\n",padapter->pid[1]);
// alio		rtw_signal_process(padapter->pid[1], SIGUSR2);
	}
