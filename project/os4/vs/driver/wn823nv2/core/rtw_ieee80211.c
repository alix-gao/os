...
-----------------------------------------------------------------------------*/
u8 *rtw_get_ie(u8 *pbuf, sint index, s32 *len, u32 limit)
{
	sint tmp,i;
	u8 *p;
_func_enter_;
 ...
 */
u8 *rtw_get_ie_ex(u8 *in_ie, uint in_len, u8 eid, u8 *oui, u8 oui_len, u8 *ie, u32 *ielen)
{
	uint cnt;
	u8 *target_ie = NULL;
...
unsigned char *rtw_get_wpa_ie(unsigned char *pie, s32 *wpa_ie_len, int limit)
{
	s32 len;
	u16 val16;
...
unsigned char *rtw_get_wpa2_ie(unsigned char *pie, s32 *rsn_ie_len, int limit)
{

	return rtw_get_ie(pie, _WPA2_IE_ID_,(s32 *)rsn_ie_len, limit);
...
u8 *rtw_get_wps_ie_from_scan_queue(u8 *in_ie, uint in_len, u8 *wps_ie, uint *wps_ielen, u8 frame_type)
{
	...
	switch( frame_type )
	{
		...
		{	//	Beacon or Probe Response
			wps = rtw_get_wps_ie(in_ie + _PROBERSP_IE_OFFSET_, in_len - _PROBERSP_IE_OFFSET_, wps_ie, (u32 *) wps_ielen);
			break;
		}
		case 2:
		{	//	Probe Request
			wps = rtw_get_wps_ie(in_ie + _PROBEREQ_IE_OFFSET_ , in_len - _PROBEREQ_IE_OFFSET_ , wps_ie, (u32 *) wps_ielen);
			break;
		}
	}
	return wps;
 ...
 */
u8 *rtw_get_wps_ie(u8 *in_ie, uint in_len, u8 *wps_ie, u32 *wps_ielen)
{
	uint cnt;
	u8 *wpsie_ptr = NULL;
 ...
 */
u8 *rtw_get_wps_attr_content(u8 *wps_ie, uint wps_ielen, u16 target_attr_id ,u8 *buf_content, u32 *len_content)
{
	u8 *attr_ptr;
	u32 attr_len;
...
u8 *rtw_get_p2p_ie_from_scan_queue(u8 *in_ie, int in_len, u8 *p2p_ie, u32 *p2p_ielen, u8 frame_type)
{
	u8*	p2p = NULL;
	...
	switch( frame_type )
	{
		...
		case 3:
		{	//	Beacon or Probe Response
			p2p = rtw_get_p2p_ie(in_ie + _PROBERSP_IE_OFFSET_, in_len - _PROBERSP_IE_OFFSET_, p2p_ie, (u32 *) p2p_ielen);
			break;
		}
		case 2:
		{	//	Probe Request
			p2p = rtw_get_p2p_ie(in_ie + _PROBEREQ_IE_OFFSET_ , in_len - _PROBEREQ_IE_OFFSET_ , p2p_ie, (u32 *) p2p_ielen);
			break;
		}
	}
	return p2p;
 ...
 */
u8 *rtw_get_p2p_ie(u8 *in_ie, int in_len, u8 *p2p_ie, u32 *p2p_ielen)
{
	uint cnt;
	u8 *p2p_ie_ptr = NULL;
 ...
 */
u8 *rtw_get_p2p_attr_content(u8 *p2p_ie, uint p2p_ielen, u8 target_attr_id ,u8 *buf_content, u32 *len_content)
{
	u8 *attr_ptr;
	u32 attr_len;
...
int rtw_get_wfd_ie(u8 *in_ie, int in_len, u8 *wfd_ie, u32 *wfd_ielen)
{
	int match;
	uint cnt = 0;
...
int rtw_get_wfd_ie_from_scan_queue(u8 *in_ie, int in_len, u8 *wfd_ie, u32 *wfd_ielen, u8 frame_type)
{
	int match;
	...
	switch( frame_type )
	{
		...
		case 3:
		{	//	Beacon or Probe Response
			match = rtw_get_wfd_ie(in_ie + _PROBERSP_IE_OFFSET_, in_len - _PROBERSP_IE_OFFSET_, wfd_ie, (u32 *) wfd_ielen);
			break;
		}
		case 2:
		{	//	Probe Request
			match = rtw_get_wfd_ie(in_ie + _PROBEREQ_IE_OFFSET_ , in_len - _PROBEREQ_IE_OFFSET_ , wfd_ie, (u32 *) wfd_ielen);
			break;
		}
	}
	return match;
...
int rtw_get_wfd_attr_content(u8 *wfd_ie, uint wfd_ielen, u8 target_attr_id ,u8 *attr_content, u32 *attr_contentlen)
{
	int match;
	uint cnt = 0;
