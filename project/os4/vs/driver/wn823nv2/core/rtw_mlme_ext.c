...
{
	s32	seq, len, status, algthm, offset;
	unsigned char	*p;
...
#endif
	{
		DBG_871X_LEVEL(_drv_always_, FUNC_ADPT_FMT" reason=%d, ta=%pM\n"
			, FUNC_ADPT_ARG(padapter), reason, GetAddr2Ptr(pframe));
...
int _issue_probereq(_adapter *padapter, NDIS_802_11_SSID *pssid, u8 *da, u8 ch, bool append_wps, int wait_ack)
{
	...
	*(fctrl) = 0;
	...
	if(pssid) {
	    if (32 < pssid->SsidLength) print("%d\n", pssid->SsidLength);
...
unsigned int send_delba_sta_tid(_adapter *adapter, u8 initiator, struct sta_info *sta, u8 tid
	, u8 force)
{
	return _send_delba_sta_tid(adapter, initiator, sta, tid, force, 0);
}

unsigned int send_delba_sta_tid_wait_ack(_adapter *adapter, u8 initiator, struct sta_info *sta, u8 tid
	, u8 force)
{
...
void site_survey(_adapter *padapter)
{
	...
	if (1){
#ifdef CONFIG_P2P
		DBG_871X(FUNC_ADPT_FMT" ch:%d (cnt:%d,idx:%d) at %dms, %c%c%c\n"
		, FUNC_ADPT_ARG(padapter)
		, survey_channel
...
#ifdef CONFIG_P2P
        flog("%s %d %d\n", __FUNCTION__, rtw_p2p_chk_state(pwdinfo, P2P_STATE_SCAN), rtw_p2p_chk_state(pwdinfo, P2P_STATE_FIND_PHASE_SEARCH));
...
u8 collect_bss_info(_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
	...
	if (p)
	{
		...
		flog("\n%s dsconfig %d, %d\n", bssid->Ssid.Ssid, bssid->Configuration.DSConfig, rtw_get_oper_ch(padapter));
	}
	else
	...
	if(str_cmp(bssid->Ssid.Ssid, DBG_RX_SIGNAL_DISPLAY_SSID_MONITORED) == 0) {
...
void start_clnt_auth(_adapter* padapter)
{
	...
	set_link_timer(pmlmeext, REAUTH_TO);
	DBG_871X_LEVEL(_drv_always_, "reauth timer started\n");
}
...
void start_clnt_assoc(_adapter* padapter)
{
    ...
    flog("timer left %d\n", timer_left(padapter->mlmepriv.assoc_timer.handle));
	...
	if (is_client_associated_to_ap(padapter))
	{
		...
		if ((psta = rtw_get_stainfo(pstapriv, pmlmeinfo->network.MacAddress)) != NULL)
		{
			...
			if (pmlmeext->active_keep_alive_check && (rx_chk == _FAIL || tx_chk == _FAIL)) {
                ...
                print("(%d.%d:%d %d %d %d)", pxmitpriv->last_tx_pkts, pxmitpriv->tx_pkts,
                                             tx_chk != _SUCCESS, pmlmeinfo->link_count, link_count_limit, rx_chk != _SUCCESS);
...
void survey_timer_hdl(_adapter *padapter)
{
	...
	DBG_871X("marc: survey timer %x\n", pmlmeext->sitesurvey_res.state);
...
u8 sitesurvey_cmd_hdl(_adapter *padapter, u8 *pbuf)
{
    ...
    flog("sitesurvey_cmd_hdl, state:%x\n", pmlmeext->sitesurvey_res.state);
