		    ...
		    u8 *ptk, size_t ptk_len)
{
	...
	if (memcmp(addr1, addr2, ETH_ALEN) < 0) {
		mem_cpy(data, addr1, ETH_ALEN);
		mem_cpy(data + ETH_ALEN, addr2, ETH_ALEN);
	} else {
		mem_cpy(data, addr2, ETH_ALEN);
		mem_cpy(data + ETH_ALEN, addr1, ETH_ALEN);
	}

	if (memcmp(nonce1, nonce2, WPA_NONCE_LEN) < 0) {
		mem_cpy(data + 2 * ETH_ALEN, nonce1, WPA_NONCE_LEN);
		mem_cpy(data + 2 * ETH_ALEN + WPA_NONCE_LEN, nonce2,
			  WPA_NONCE_LEN);
	} else {
		mem_cpy(data + 2 * ETH_ALEN, nonce2, WPA_NONCE_LEN);
		mem_cpy(data + 2 * ETH_ALEN + WPA_NONCE_LEN, nonce1,
			  WPA_NONCE_LEN);
	}
	       ...
	       const u8 *ric, size_t ric_len, u8 *mic)
{
	...
	pos = buf;
	mem_cpy(pos, sta_addr, ETH_ALEN);
	pos += ETH_ALEN;
	mem_cpy(pos, ap_addr, ETH_ALEN);
	pos += ETH_ALEN;
	*pos++ = transaction_seqnum;
	if (rsnie) {
		mem_cpy(pos, rsnie, rsnie_len);
		pos += rsnie_len;
	}
	if (mdie) {
		mem_cpy(pos, mdie, mdie_len);
		pos += mdie_len;
	}
	if (ftie) {
		struct rsn_ftie *_ftie;
		mem_cpy(pos, ftie, ftie_len);
		if (ftie_len < 2 + sizeof(*_ftie)) {
			os_free(buf);
			return -1;
		}
		_ftie = (struct rsn_ftie *) (pos + 2);
		mem_set(_ftie->mic, 0, sizeof(_ftie->mic));
		pos += ftie_len;
	}
	if (ric) {
		mem_cpy(pos, ric, ric_len);
		pos += ric_len;
	}
		       ...
		       const u8 *s0kh_id, u8 *pmk_r0, u8 *pmk_r0_name)
{
	...
	pos = buf;
	*pos++ = ssid_len;
	mem_cpy(pos, ssid, ssid_len);
	pos += ssid_len;
	mem_cpy(pos, mdid, MOBILITY_DOMAIN_ID_LEN);
	pos += MOBILITY_DOMAIN_ID_LEN;
	*pos++ = r0kh_id_len;
	mem_cpy(pos, r0kh_id, r0kh_id_len);
	pos += r0kh_id_len;
	mem_cpy(pos, s0kh_id, ETH_ALEN);
	pos += ETH_ALEN;

	sha256_prf(xxkey, xxkey_len, "FT-R0", buf, pos - buf,
		   r0_key_data, sizeof(r0_key_data));
	mem_cpy(pmk_r0, r0_key_data, PMK_LEN);
	...
	mem_cpy(pmk_r0_name, hash, WPA_PMK_NAME_LEN);
}
			    ...
			    const u8 *s1kh_id, u8 *pmk_r1_name)
{
	...
	mem_cpy(pmk_r1_name, hash, WPA_PMK_NAME_LEN);
}
		       ...
		       u8 *pmk_r1, u8 *pmk_r1_name)
{
	...
	pos = buf;
	mem_cpy(pos, r1kh_id, FT_R1KH_ID_LEN);
	pos += FT_R1KH_ID_LEN;
	mem_cpy(pos, s1kh_id, ETH_ALEN);
	pos += ETH_ALEN;
		       ...
		       u8 *ptk, size_t ptk_len, u8 *ptk_name)
{
	 ...
	 */
	pos = buf;
	mem_cpy(pos, snonce, WPA_NONCE_LEN);
	pos += WPA_NONCE_LEN;
	mem_cpy(pos, anonce, WPA_NONCE_LEN);
	pos += WPA_NONCE_LEN;
	mem_cpy(pos, bssid, ETH_ALEN);
	pos += ETH_ALEN;
	mem_cpy(pos, sta_addr, ETH_ALEN);
	pos += ETH_ALEN;
	...
	mem_cpy(ptk_name, hash, WPA_PMK_NAME_LEN);
}
...
os_u8 * wpa_bss_get_vendor_ie(os_u8 *bss, os_uint len, u32 vendor_type)
{
	os_u8 *end, *pos;
...
os_u8 * wpa_bss_get_ie(os_u8 *bss, os_uint len, u8 ie)
{
	os_u8 *end, *pos;
...
void select_first_bitfield(int *cipher)
{
    int mask;
// TODO: select alg
    //for (mask = 1; mask; mask = mask << 1) {
    for (mask = (1 << 15); mask; mask = mask >> 1) {
        if (mask & (*cipher)) {
            *cipher = mask;
            return;
        }
...
os_u8 security_ie[0x100];
os_uint security_ie_len;
struct wpa_ie_data wpa_cfg;
extern struct wpa_dev *dbg_wpa;
os_ret gen_ie_by_parse_scan(os_u8 *buf, os_uint len, os_u8 *beacon_ie, os_uint beacon_ie_len)
{
    os_u8 *ie, *ie2;
    int protocol;

    if ((OS_NULL == buf) || (0 == len)) {
        return OS_FAIL;
    }
// TODO: select alg
    ie2 = wpa_bss_get_ie(beacon_ie + _FIXED_IE_LENGTH_, beacon_ie_len - _FIXED_IE_LENGTH_, WLAN_EID_RSN);
    if (ie2) {
        wpa_parse_wpa_ie(ie2, 2 + ie2[1], &wpa_cfg);
        protocol = WPA_PROTO_RSN;
        goto gen_ie;
    }
    ie = wpa_bss_get_vendor_ie(beacon_ie + _FIXED_IE_LENGTH_, beacon_ie_len - _FIXED_IE_LENGTH_, WPA_IE_VENDOR_TYPE);
    if (ie) {
        wpa_parse_wpa_ie(ie, 2 + ie[1], &wpa_cfg);
        protocol = WPA_PROTO_WPA;
        goto gen_ie;
    }

    wpa_printf(MSG_ERROR, "gen_ie_by_parse_scan fail\n");
    return OS_FAIL;
  gen_ie:
    wpa_printf(MSG_ERROR, "%d pcipher %x, gcipher %x, kmgmt %x\n", protocol, wpa_cfg.pairwise_cipher, wpa_cfg.group_cipher, wpa_cfg.key_mgmt);
    select_first_bitfield(&wpa_cfg.pairwise_cipher);
    select_first_bitfield(&wpa_cfg.group_cipher);
    select_first_bitfield(&wpa_cfg.key_mgmt);
    wpa_printf(MSG_ERROR, "%d pcipher %x, gcipher %x, kmgmt %x\n", protocol, wpa_cfg.pairwise_cipher, wpa_cfg.group_cipher, wpa_cfg.key_mgmt);
    wpa_gen_wpa_ie(buf, len, protocol, wpa_cfg.pairwise_cipher, wpa_cfg.group_cipher, wpa_cfg.key_mgmt, wpa_cfg.mgmt_group_cipher);
    security_ie_len = 2 + buf[1];
    mem_cpy(security_ie, buf, security_ie_len);
    mem_cpy(&dbg_wpa->wpa_cfg, &wpa_cfg, sizeof(struct wpa_ie_data));
    return OS_SUCC;
}
