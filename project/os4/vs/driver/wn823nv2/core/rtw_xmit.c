...
void update_attrib_phy_info(_adapter *padapter, struct pkt_attrib *pattrib, struct sta_info *psta)
{
...
s32 update_attrib_sec_info(_adapter *padapter, struct pkt_attrib *pattrib, struct sta_info *psta)
{
	sint res = _SUCCESS;
...
static s32 update_attrib(_adapter *padapter, _pkt *pkt, struct pkt_attrib *pattrib)
{
 ...
 _func_enter_;
    ...
    flog("%x %x %x\n", pkt, pkt->data[0], pkt->data[1]);
    flog("h_dest:%x %x %x %x %x %x\n", etherhdr.h_dest[0], etherhdr.h_dest[1], etherhdr.h_dest[2], etherhdr.h_dest[3], etherhdr.h_dest[4], etherhdr.h_dest[5]);
...
int rtw_br_client_tx(_adapter *padapter, struct sk_buff **pskb)
{
	...
	//if(check_fwstate(pmlmepriv, WIFI_STATION_STATE|WIFI_ADHOC_STATE) == _TRUE)
	{
		...
		if (	!(skb->data[0] & 1) &&
			...
			mem_cpy(skb->data+MACADDRLEN, GET_MY_HWADDR(padapter), MACADDRLEN);
...
//				 !memcmp(skb->data+MACADDRLEN, priv->br_mac, MACADDRLEN)) {
			...
			if (!memcmp(skb->data+MACADDRLEN, padapter->br_mac, MACADDRLEN) &&
				(*((unsigned short *)(skb->data+MACADDRLEN*2)) == __constant_htons(ETH_P_IP)))
				mem_cpy(padapter->br_ip, skb->data+WLAN_ETHHDR_LEN+12, 4);
			...
			if (*((unsigned short *)(skb->data+MACADDRLEN*2)) == __constant_htons(ETH_P_IP)) {
				if (memcmp(padapter->scdb_mac, skb->data+MACADDRLEN, MACADDRLEN)) {
					...
					if ((padapter->scdb_entry = (struct nat25_network_db_entry *)scdb_findEntry(padapter,
						...
						mem_cpy(padapter->scdb_mac, skb->data+MACADDRLEN, MACADDRLEN);
						mem_cpy(padapter->scdb_ip, skb->data+WLAN_ETHHDR_LEN+12, 4);
						padapter->scdb_entry->ageing_timer = jiffies;
						do_nat25 = 0;
					}
				}
					...
					if (padapter->scdb_entry) {
						...
						mem_set(padapter->scdb_mac, 0, MACADDRLEN);
						mem_set(padapter->scdb_ip, 0, 4);
					}
				}
			}
			_exit_critical_bh(&padapter->br_ext_lock, &irqL);
...
// alic	init_completion(&sctx->done);
#endif
...
s32 rtw_xmit_eapol_coalesce(_adapter * padapter, struct xmit_frame * pxmitframe, u8 *data, uint len)
{
	s32 llc_sz;
...
_func_enter_;
	...
	pattrib->nr_frags = 1;
	pattrib->last_txcmdsz = pattrib->hdrlen + pattrib->iv_len + llc_sz + len +
			((pattrib->bswenc) ? pattrib->icv_len : 0) + pattrib->pktlen;
