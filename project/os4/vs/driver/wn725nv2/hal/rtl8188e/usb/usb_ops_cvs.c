...
void rtl8188eu_recv_tasklet(void *priv)
{
	...
	while (NULL != (pskb = skb_dequeue(&precvpriv->rx_skb_queue)))
	{
		if ((padapter->bDriverStopped == _TRUE)||(padapter->bSurpriseRemoved== _TRUE))
		{
			...
			rtw_skb_free(pskb);
#else
            skb_queue_head(&precvpriv->rx_skb_queue, pskb);
#endif
			break;
		}
...
static void usb_read_port_complete(struct recv_buf	*precvbuf, int status, u32 actual_length)
{
	_irqL irqL;
	uint isevt, *pbuf;
	// alic struct recv_buf	*precvbuf = (struct recv_buf *)purb->context;
	_adapter 			*padapter =(_adapter *)precvbuf->adapter;
	struct recv_priv	*precvpriv = &padapter->recvpriv;
	...
	if(status==0)//SUCCESS
	{
		...
		if ((actual_length > MAX_RECVBUF_SZ) || (actual_length < RXDESC_SIZE))
		{
			RT_TRACE(_module_hci_ops_os_c_,_drv_err_,("usb_read_port_complete: (purb->actual_length > MAX_RECVBUF_SZ) || (purb->actual_length < RXDESC_SIZE)\n"));
			precvbuf->reuse = _TRUE;
			rtw_read_port(padapter, precvpriv->ff_hwaddr, 0, (unsigned char *)precvbuf);
		...
		else
		{
			...
			precvbuf->transfer_len = actual_length;
			skb_put(precvbuf->pskb, actual_length);
...
flog("%d %x ", precvbuf->reuse, precvbuf->pskb);
#ifdef CONFIG_PREALLOC_RECV_SKB
    ...
    usb_read_port_complete(precvbuf, err, MAX_RECVBUF_SZ);
#endif
	if((err) && (err != (-EPERM)))
	{
		RT_TRACE(_module_hci_ops_os_c_,_drv_err_,("cannot submit rx in-token(err=0x%.8x), URB_STATUS =0x%.8x", err, purb->status));
