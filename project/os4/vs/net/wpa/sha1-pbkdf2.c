...
static int pbkdf2_sha1_f(const char *passphrase, const char *ssid,
			 uint ssid_len, int iterations, unsigned int count,
			 u8 *digest)
{
	...
	const u8 *addr[2];
	uint len[2];
	uint passphrase_len = str_len(passphrase);
	...
	if (hmac_sha1_vector((u8 *) passphrase, passphrase_len, 2, addr, len,
			     tmp))
		return -1;
	mem_cpy(digest, tmp, SHA1_MAC_LEN);

	for (i = 1; i < iterations; i++) {
		if (hmac_sha1((u8 *) passphrase, passphrase_len, tmp,
			      SHA1_MAC_LEN, tmp2))
			return -1;
		mem_cpy(tmp, tmp2, SHA1_MAC_LEN);
