	...
	)
{
	...
	DBG_871X("%s OutEpQueueSel(0x%x), OutEpNumber(%d) \n",__FUNCTION__,pHalData->OutEpQueueSel,pHalData->OutEpNumber );
...
u32 rtl8192eu_inirp_init(PADAPTER Adapter)
{
	u8 i;
...
#ifdef CONFIG_USB_INTERRUPT_IN_PIPE
/* TDLS TODO: the following would let compile error */
#if 0
...
u32 rtl8192eu_inirp_deinit(PADAPTER Adapter)
{
	...
	)
{
	...
	DBG_871X("VID = 0x%x, PID = 0x%x\n", pHalData->EEPROMVID, pHalData->EEPROMPID);
	DBG_871X("Customer ID: 0x%x, SubCustomer ID: 0x%x\n", pHalData->EEPROMCustomerID, pHalData->EEPROMSubCustomerID);
}
	...
	)
{
	...
	Hal_ReadRFEType_8192E(Adapter, pEEPROM->efuse_eeprom_data, pEEPROM->bautoload_fail_flag);
