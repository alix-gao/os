...
#endif
static os_ret rtw_drv_init(HDEVICE usb);
static void rtw_dev_remove(HDEVICE usb);
static void rtw_suspend(HDEVICE pusb_intf);
static void rtw_resume(HDEVICE pusb_intf);

LOCALC void *usb_get_intfdata(struct usb_device *intf)
{
    return get_usb_dedicated(intf);
}

static void rtw_dev_shutdown(struct usb_device *intf)
{
...
static struct usb_device_info rtw_usb_id_tbl[] ={
...
// alic MODULE_DEVICE_TABLE(usb, rtw_usb_id_tbl);
...
LOCALD const struct usb_device_id wn823n_id = {
    USB_VENDER_ID_REALTEK, 0x818B,
    USB_ANY_ID,
    USB_ANY_ID,
    USB_ANY_ID
};
...
struct rtw_usb_drv usb_drv = {
	...
	.usbdrv.add_device = rtw_drv_init,
	.usbdrv.del_device = rtw_dev_remove,
	.usbdrv.id = &wn823n_id, //rtw_usb_id_tbl,
	// alic .usbdrv.suspend =  rtw_suspend,
	// alic .usbdrv.resume = rtw_resume,
	...
	#else
// alic	.usbdrv.driver.shutdown = rtw_dev_shutdown,
	#endif
};
...
static struct dvobj_priv *usb_dvobj_init(struct usb_device *usb)
{
	int	i;
	u8	val8;
	int	status = _FAIL;
...
//	pdvobjpriv->pusbintf = usb;
	pusbd = pdvobjpriv->pusbdev = usb;
//	usb_set_intfdata(usb_intf, pdvobjpriv);
    set_usb_dedicated(usb, pdvobjpriv);
	...
	pdev_desc = usb_device_desc(pusbd);
...
//	piface_desc = &phost_iface->desc;
...
#else
    pconf_desc = usb_config_desc(usb);
    intf_info = usb_interface_info(usb);
    pdvobjpriv->NumInterfaces = pconf_desc->bNumInterfaces;
    pdvobjpriv->InterfaceNumber = intf_info->interface->bInterfaceNumber;
    pdvobjpriv->nr_endpoint = intf_info->interface->bNumEndpoints; // alic piface_desc->bNumEndpoints;
#endif
	...
	{
		pendp_desc = intf_info->endpoint[i];
...
static void usb_dvobj_deinit(struct usb_device *usb_intf)
{
...
//	usb_set_intfdata(usb_intf, NULL);
    set_usb_dedicated(usb_intf, OS_NULL);
	if (dvobj) {
...
static void rtw_decide_chip_type_by_usb_info(_adapter *padapter, struct usb_device *usb)
{
    os_u32 i;

    for (i = 0; i < array_size(rtw_usb_id_tbl); i++) {
        if ((usb_device_desc(usb)->idProduct == rtw_usb_id_tbl[i].idProduct)
         && (usb_device_desc(usb)->idVendor == rtw_usb_id_tbl[i].idVendor)) {
            padapter->chip_type = rtw_usb_id_tbl[i].driver_info;
            break;
        }
    }
    if (array_size(rtw_usb_id_tbl) <= i) {
        cassert(FALSE);
    }
...
static void process_spec_devid(os_u16 idProduct, os_u16 idVendor)
{
	u16 vid, pid;
	u32 flags;
	int i;
...
#ifdef CONFIG_80211N_HT
		if((idVendor==vid) && (idProduct==pid) && (flags&SPEC_DEV_ID_DISABLE_HT))
		{
			 rtw_ht_enable = 0;
			 rtw_bw_mode = 0;
			 rtw_ampdu_enable = 0;
...
//	struct net_device *pnetdev;
	...
	if((_TRUE == padapter->bDriverStopped)||(_TRUE==padapter->bSurpriseRemoved))
	{
...
//	struct net_device *pnetdev = padapter->pnetdev;
	...
	if(pm_netdev_open(padapter,_FALSE) != 0)
	{
		_exit_pwrlock(&pwrpriv->lock);
		goto error_exit;
	}
	...
	struct usb_device *usb)
{
	...
	rtw_decide_chip_type_by_usb_info(padapter, usb);
...
#else
    pnetdev = kmalloc(sizeof(struct net_device));
    if (NULL == pnetdev) {
        goto handle_dualmac;
    }
...
static os_ret rtw_drv_init(HDEVICE usb)
{
	...
	RT_TRACE(_module_hci_intfs_c_, _drv_err_, ("823nv2+rtw_drv_init\n"));
	//DBG_871X("+rtw_drv_init\n");

	//step 0.
	process_spec_devid(usb_device_desc(usb)->idProduct, usb_device_desc(usb)->idVendor);

	/* Initialize dvobj_priv */
	if ((dvobj = usb_dvobj_init(usb)) == NULL) {
		RT_TRACE(_module_hci_intfs_c_, _drv_err_, ("initialize device object priv Failed!\n"));
		goto exit;
	}

	if ((if1 = rtw_usb_if1_init(dvobj, usb)) == NULL) {
	...
	if(ui_pid[1]!=0) {
		DBG_871X("ui_pid[1]:%d\n",ui_pid[1]);
// alic		rtw_signal_process(ui_pid[1], SIGUSR2);
	}
#endif
    ...
    create_wpa_dev(if1); // added by alic

	status = _SUCCESS;print("rtw drv init end\n");
...
static void rtw_dev_remove(HDEVICE usb)
{
	struct dvobj_priv *dvobj = usb_get_intfdata(usb);
...
_func_enter_;
	...
	Eth_DeRegDevice(padapter->dev_id); // added by alic
...
static void rtw_drv_entry(void)
{
	int ret = 0;
	...
	ret = register_usb_driver(&usb_drv.usbdrv);
	if (ret != 0) {
...
init_driver_call(rtw_drv_entry);
// alic module_exit(rtw_drv_halt);
