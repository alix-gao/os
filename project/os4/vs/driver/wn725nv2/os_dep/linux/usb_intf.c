...
//static int rtw_drv_init(struct usb_interface *pusb_intf,const struct usb_device_id_t *pdid);
...
/* Some useful macros to use to create struct usb_device_id_t */
...
#define USB_DEVICE_AND_INTERFACE_INFO(vend, prod, cl, sc, pr) \
  ...
  *
  * This macro is used to create a struct usb_device_id_t that matches a
...
static struct usb_device_id_t rtw_usb_id_tbl[] ={
...
int const rtw_usb_id_len = sizeof(rtw_usb_id_tbl) / sizeof(struct usb_device_id_t);
...
LOCALD const struct usb_device_id wn725n_id = {
    USB_VENDER_ID_REALTEK, 0x8179,
    USB_ANY_ID,
    USB_ANY_ID,
    USB_ANY_ID
};
...
static struct dvobj_priv *usb_dvobj_init(HDEVICE usb)//(struct usb_interface *usb_intf)
{
	int	i;
	u8	val8;
	int	status = _FAIL;
...
//	pdvobjpriv->pusbintf = usb_intf ;
	pusbd = pdvobjpriv->pusbdev = interface_to_usbdev(usb);
	usb_set_intfdata(usb, pdvobjpriv);
...
//	pdvobjpriv->nr_endpoint = piface_desc->bNumEndpoints;
    ...
    pdvobjpriv->nr_endpoint = intf_info->interface->bNumEndpoints;
...
//			pendp_desc = &phost_endp->desc;
    ...
    static const os_u8 usb_speed_covert[USB_SPEED_COUNT] = {
        USB_SPEED_LOW, USB_SPEED_FULL,
        USB_SPEED_HIGH
    };
	switch(usb_speed_covert[usb_device_speed(pusbd)]) {
		case USB_SPEED_LOW:
...
static void usb_dvobj_deinit(struct usb_device *usb)
{
...
_func_enter_;
	...
	usb_put_dev(interface_to_usbdev(usb));

_func_exit_;
}

static void rtw_decide_chip_type_by_usb_info(_adapter *padapter, os_u16 idVendor, os_u16 idProduct)
{
    os_u32 i;

    for (i = 0; i < array_size(rtw_usb_id_tbl); i++) {
        if ((idVendor == rtw_usb_id_tbl[i].idVendor)
         && (idProduct == rtw_usb_id_tbl[i].idProduct)) {
            padapter->chip_type = rtw_usb_id_tbl[i].driver_info;
            break;
        }
    }
    if (!(i < array_size(rtw_usb_id_tbl))) {
        flog("error %d, rtw_decide_chip_type_by_usb_info, %x %x\n", i, idVendor, idProduct);
        return;
    }
...
static void process_spec_devid(os_u16 idVendor, os_u16 idProduct)
{
	u16 vid, pid;
	u32 flags;
	int i;
...
_adapter *rtw_usb_if1_init(struct dvobj_priv *dvobj,
	struct usb_device *usb, os_u16 idVendor, os_u16 idProduct)
{
	...
	rtw_decide_chip_type_by_usb_info(padapter, idVendor, idProduct);
	...
	#if (LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,33))
			if (usb_autopm_get_interface(usb) < 0)
				{
...
static os_ret rtw_drv_init(HDEVICE usb)//(struct usb_interface *pusb_intf, const struct usb_device_id_t *pdid)
{
	...
	//step 0.
	process_spec_devid(usb_device_desc(usb)->idVendor, usb_device_desc(usb)->idProduct);
	...
	if ((if1 = rtw_usb_if1_init(dvobj, usb, usb_device_desc(usb)->idVendor, usb_device_desc(usb)->idProduct)) == NULL) {
...
static void rtw_dev_remove(HDEVICE usb)//(struct usb_interface *pusb_intf)
{
	struct dvobj_priv *dvobj = usb_get_intfdata(usb);
...
_func_enter_;
    ...
    del_netif(padapter); // added by alic
...
LOCALC void rtw_drv_init_wrap(void)
{
    rtw_drv_entry();
}
init_driver_call(rtw_drv_init_wrap);
