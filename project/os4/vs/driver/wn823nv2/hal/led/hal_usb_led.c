...
//
os_ret OS_CALLBACK BlinkTimerCallback(os_u32 data)
{
...
// alic		_set_workitem(&(pLed->BlinkWorkItem));
	}
	#endif
}
	...
	)
{
	...
	_init_timer(&(pLed->BlinkTimer), 0, BlinkTimerCallback, pLed);
// alic	_init_workitem(&(pLed->BlinkWorkItem), BlinkWorkItemCallback, pLed);
}
...
// alic	_cancel_workitem_sync(&(pLed->BlinkWorkItem));
	_cancel_timer_ex(&(pLed->BlinkTimer));
	ResetLedStatus(pLed);
}
