...
)
{
	...
	if (input_num) {
		...
		for (i = 0; i < n; i++) {
			if (str_cmp(phy_dm_ary[i].name, input[0]) == 0) {
				id = phy_dm_ary[i].id;
				break;
			}
		}
	...
	case PHYDM_DEBUG:
		...
		if (input_idx >= 1) {
			...
			odm_debug_trace(pDM_Odm, (IN u4Byte *const) var1, &used, output, &out_len);
		}
	...
	case PHYDM_SUPPORT_ABLITY:
		...
		if (input_idx >= 1) {
			...
			phydm_support_ablity_debug(pDM_Odm, (IN u4Byte *const) var1, &used, output, &out_len);
		}
...
)
{
	...
	do {
		token = str_brk((u8 **) &input, ", ");
		if (token) {
			str_cpy(Argv[Argc], token);
			Argc++;
		} else
			break;
	} while (Argc < MAX_ARGC);
