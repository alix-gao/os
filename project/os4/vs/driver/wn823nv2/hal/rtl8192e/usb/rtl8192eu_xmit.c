...
os_ret OS_CALLBACK xmit_tasklet_entry_point(os_u32 arg1, os_u32 arg2, os_u32 arg3, os_u32 arg4, os_u32 arg5, os_u32 arg6, os_u32 arg7)
{
    _adapter *padapter;

    padapter = (_adapter *) arg1;
    while (1) {
        delay_task(0x4, __LINE__);
        rtl8192eu_xmit_tasklet(padapter);
    }
}
...
s32	rtl8192eu_init_xmit_priv(_adapter *padapter)
{
    ...
    do {
        HTASK handle;
        handle = create_task("usb_xmit_tasklet", xmit_tasklet_entry_point, RTW_DRIVER_TX_PRI, (os_u32) padapter, 0, 0, 0, 0, 0, 0);
...
s32 rtl8192eu_xmit_buf_handler(PADAPTER padapter)
{
    ...
    flog("rtl8192eu_xmit_buf_handler\n");
...
flog("check pending start\n");
	if(check_pending_xmitbuf(pxmitpriv) == _FALSE)
		return _SUCCESS;
flog("check pending end\n");
...
static s32 rtw_dump_xframe(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	mem_addr = pxmitframe->buf_addr;
       ...
       RT_TRACE(_module_rtl871x_xmit_c_,_drv_info_,("rtw_dump_xframe() %d\n", pattrib->nr_frags));
...
flog("priority %d\n", get_task_priority(current_task_handle()));
	if  (ret != _SUCCESS)
