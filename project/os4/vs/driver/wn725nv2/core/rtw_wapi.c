...
//protocol version
	mem_cpy(pWapiInfo->wapiIE+pWapiInfo->wapiIELength, &protocolVer, 2);
	pWapiInfo->wapiIELength +=2;
//akm
	mem_cpy(pWapiInfo->wapiIE+pWapiInfo->wapiIELength, &akmCnt, 2);
	pWapiInfo->wapiIELength +=2;

	if(pWapiInfo->bWapiPSK){
		mem_cpy(pWapiInfo->wapiIE+pWapiInfo->wapiIELength,OUI, 3);
...
//usk
	mem_cpy(pWapiInfo->wapiIE+pWapiInfo->wapiIELength, &suiteCnt, 2);
	pWapiInfo->wapiIELength +=2;
	mem_cpy(pWapiInfo->wapiIE+pWapiInfo->wapiIELength,OUI, 3);
	pWapiInfo->wapiIELength +=3;
	pWapiInfo->wapiIE[pWapiInfo->wapiIELength] = 0x1;
...
//Capbility
	mem_cpy(pWapiInfo->wapiIE+pWapiInfo->wapiIELength, &capability, 2);
	pWapiInfo->wapiIELength +=2;
}
...
void rtw_wapi_return_one_sta_info(_adapter *padapter, u8 *MacAddr)
{
	...
	if(list_empty(&(pWapiInfo->wapiSTAUsedList))){
		...
		while(!list_empty(&(pWapiInfo->wapiSTAUsedList)))
		{
			...
			list_del_init(&pWapiStaInfo->list);
			mem_set(pWapiStaInfo->PeerMacAddr,0,ETH_ALEN);
...
{ /*Data: keyType(1) + bTxEnable(1) + bAuthenticator(1) + bUpdate(1) + PeerAddr(6) + DataKey(16) + MicKey(16) + KeyId(1)*/
	...
	bUpdate = data[3];
 	mem_cpy(PeerAddr,data+4,6);

	if(data[0] == 0x3){
		if(!list_empty(&(pWapiInfo->wapiBKIDIdleList))){
			pWapiBkid = (PRT_WAPI_BKID)list_entry(pWapiInfo->wapiBKIDIdleList.next, RT_WAPI_BKID, list);
			list_del_init(&pWapiBkid->list);
			mem_cpy(pWapiBkid->bkid, data+10, 16);
			...
			if(!memcmp(pWapiSta->PeerMacAddr,PeerAddr,6)){
				...
				case 1:              //usk
					if(bAuthenticator){         //authenticator
						mem_cpy(pWapiSta->lastTxUnicastPN,WapiAEPNInitialValueSrc,16);
						if(!bUpdate) {     //first
							WAPI_TRACE(WAPI_INIT,"AE fisrt set usk \n");
							pWapiSta->wapiUsk.bSet = true;
							mem_cpy(pWapiSta->wapiUsk.dataKey,data+10,16);
							mem_cpy(pWapiSta->wapiUsk.micKey,data+26,16);
						...
						else               //update
						{
							...
							mem_cpy(pWapiSta->wapiUskUpdate.dataKey,data+10,16);
							mem_cpy(pWapiSta->wapiUskUpdate.micKey,data+26,16);
							mem_cpy(pWapiSta->lastRxUnicastPNBEQueue,WapiASUEPNInitialValueSrc,16);
							mem_cpy(pWapiSta->lastRxUnicastPNBKQueue,WapiASUEPNInitialValueSrc,16);
							mem_cpy(pWapiSta->lastRxUnicastPNVIQueue,WapiASUEPNInitialValueSrc,16);
							mem_cpy(pWapiSta->lastRxUnicastPNVOQueue,WapiASUEPNInitialValueSrc,16);
							mem_cpy(pWapiSta->lastRxUnicastPN,WapiASUEPNInitialValueSrc,16);
						...
						if(!bUpdate){
							...
							if(bTxEnable){
								pWapiSta->wapiUsk.bTxEnable = true;
								mem_cpy(pWapiSta->lastTxUnicastPN,WapiASUEPNInitialValueSrc,16);
							}else{
								pWapiSta->wapiUsk.bSet = true;
								mem_cpy(pWapiSta->wapiUsk.dataKey,data+10,16);
								mem_cpy(pWapiSta->wapiUsk.micKey,data+26,16);
							...
							if(bTxEnable){
								...
								if(pWapiSta->wapiUskUpdate.bSet){
									mem_cpy(pWapiSta->wapiUsk.dataKey,pWapiSta->wapiUskUpdate.dataKey,16);
									mem_cpy(pWapiSta->wapiUsk.micKey,pWapiSta->wapiUskUpdate.micKey,16);
				...
				case 2:		//msk
					if(bAuthenticator){          //authenticator
						...
						mem_cpy(pWapiInfo->wapiTxMsk.dataKey,data+10,16);
						mem_cpy(pWapiInfo->wapiTxMsk.micKey,data+26,16);
						pWapiInfo->wapiTxMsk.keyId = *(data+42);
						pWapiInfo->wapiTxMsk.bTxEnable = true;
						mem_cpy(pWapiInfo->lastTxMulticastPN,WapiAEMultiCastPNInitialValueSrc,16);
						...
						if(!bUpdate){
							...
							mem_cpy(pWapiSta->wapiMsk.dataKey,data+10,16);
							mem_cpy(pWapiSta->wapiMsk.micKey,data+26,16);
							...
							mem_cpy(pWapiSta->wapiMskUpdate.dataKey,data+10,16);
							mem_cpy(pWapiSta->wapiMskUpdate.micKey,data+26,16);
...
void wapi_test_init(struct _adapter *padapter)
{
	...
	mem_set(keybuf,0,100);
	keybuf[0] = 1;                           //set usk
	keybuf[1] = 1; 				//enable tx
	keybuf[2] = 1; 				//AE
	keybuf[3] = 0; 				//not update

	mem_cpy(keybuf+4,mac_addr,6);
	mem_cpy(keybuf+10,UskDataKey,16);
	mem_cpy(keybuf+26,UskMicKey,16);
	keybuf[42]=UskId;
	...
	keybuf[3] = 0;                              //not update
	mem_cpy(keybuf+4,mac_addr,6);
	mem_cpy(keybuf+10,MskDataKey,16);
	mem_cpy(keybuf+26,MskMicKey,16);
	keybuf[42] = MskId;
	wapi_test_set_key(padapter, keybuf);
...
void rtw_wapi_get_iv(_adapter *padapter,u8 *pRA, u8*IV)
{
	...
	if(IS_MCAST(pRA)){
		...
		if(pWapiInfo->wapiTxMsk.keyId <= 1){
			 ...
			 mem_cpy(pWapiExt->PN, pWapiInfo->lastTxMulticastPN, 16);
		}
	}
	else
	{
