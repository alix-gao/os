 ...
 */
int wpa_gen_wpa_ie(u8 *wpa_ie, size_t wpa_ie_len, uint proto, uint pairwise_cipher, uint group_cipher, uint key_mgmt, uint mgmt_group_cipher)
{
	if (proto == WPA_PROTO_RSN)
		return wpa_gen_wpa_ie_rsn(wpa_ie, wpa_ie_len,
					  pairwise_cipher,
					  group_cipher,
					  key_mgmt, mgmt_group_cipher);
	else
 ...
 */
static int wpa_parse_generic(u8 *pos, u8 *end,
			     struct wpa_eapol_ie_parse *ie)
{
	...
	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
		...
		wpa_hexdump(MSG_DEBUG, "WPA: SMK in EAPOL-Key",
				pos, pos[1] + 2);
		return 0;
	}
 ...
 * Returns: 0 on success, -1 on failure
 */
int wpa_supplicant_parse_ies(u8 *buf, size_t len,
			     struct wpa_eapol_ie_parse *ie)
{
	u8 *pos, *end;
	int ret = 0;
