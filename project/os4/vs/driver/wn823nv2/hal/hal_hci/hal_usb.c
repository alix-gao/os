 ...
 * history     :
 ***************************************************************/
os_ret OS_CALLBACK rcv_entry_point(os_u32 arg1, os_u32 arg2, os_u32 arg3, os_u32 arg4, os_u32 arg5, os_u32 arg6, os_u32 arg7)
{
    _adapter *padapter = (_adapter *) arg1;
    os_u32 opened_flag = OS_FALSE;

    print("recv thread\n");

    suspend_task(current_task_handle(), __LINE__);
    flog("recv thread\n");
    for (;;) {
        delay_task(0x10, __LINE__);

        //rtl8192eu_inirp_init(padapter);
        usb_recv_tasklet(padapter);
    }
    cassert(OS_FALSE);
    return OS_FAIL;
}
...
#else
    do {
        padapter->recvpriv.recv_taskhandle = create_task("usb_recv_tasklet", rcv_entry_point, RTW_DRIVER_RX_PRI, (os_u32) padapter, 0, 0, 0, 0, 0, 0);
...
#ifdef CONFIG_RX_INDICATE_QUEUE
	mem_set(&precvpriv->rx_indicate_queue, 0, sizeof(struct ifqueue));
