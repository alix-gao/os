                                                 ...
                                                 u8 *Input2, u16 Input2Length, u8 *Output, u8 *OutputLength)
{
	...
	*OutputLength = 0;
	...
	if(remainder !=0){
		mem_set(TempBlock, 0, 16);
		mem_cpy(TempBlock, (Input1+blockNum*16), remainder);
	...
	if(remainder !=0){
		mem_set(TempBlock, 0, 16);
		mem_cpy(TempBlock, (Input2+blockNum*16), remainder);

		xor_block(BlockIn, TempBlock, BlockOut);
		SMS4Crypt((u8 *)BlockIn, BlockOut, rk);
	}

	mem_cpy(Output, BlockOut, 16);
	*OutputLength = 16;
}
	...
	)
{
	...
	mem_set(TempBuf, 0, 34);
	mem_cpy(TempBuf, pHeader, 2); //FrameCtrl
	pTemp = (u16*)TempBuf;
	*pTemp &= 0xc78f;       //bit4,5,6,11,12,13

	mem_cpy((TempBuf+2), (pHeader+4), 12); //Addr1, Addr2
	mem_cpy((TempBuf+14), (pHeader+22), 2); // SeqCtrl
	pTemp = (u16*)(TempBuf + 14);
	*pTemp &= 0x000f;

	mem_cpy((TempBuf+16), (pHeader+16), 6); //Addr3
	...
	{
		mem_cpy((TempBuf+22), (pHeader+24), 6);
		QosOffset = 30;
	}else{
		mem_set((TempBuf+22), 0, 6);
		QosOffset = 24;
	}

	if((fc & 0x0088) == 0x0088){
		mem_cpy((TempBuf+28), (pHeader+QosOffset), 2);
		TempLen += 2;
...
)
{
	...
	switch(UserPriority)
	{
		case 0:
		case 3:
			      mem_cpy(PNOut,pWapiStaInfo->lastRxUnicastPNBEQueue,16);
			      break;
		case 1:
		case 2:
			      mem_cpy(PNOut,pWapiStaInfo->lastRxUnicastPNBKQueue,16);
			      break;
		case 4:
		case 5:
			      mem_cpy(PNOut,pWapiStaInfo->lastRxUnicastPNVIQueue,16);
			      break;
		case 6:
		case 7:
			      mem_cpy(PNOut,pWapiStaInfo->lastRxUnicastPNVOQueue,16);
			      break;
		default:
...
)
{
	...
	switch(UserPriority)
	{
		case 0:
		case 3:
			      mem_cpy(pWapiStaInfo->lastRxUnicastPNBEQueue,PNIn,16);
			      break;
		case 1:
		case 2:
			      mem_cpy(pWapiStaInfo->lastRxUnicastPNBKQueue,PNIn,16);
			      break;
		case 4:
		case 5:
			      mem_cpy(pWapiStaInfo->lastRxUnicastPNVIQueue,PNIn,16);
			      break;
		case 6:
		case 7:
			      mem_cpy(pWapiStaInfo->lastRxUnicastPNVOQueue,PNIn,16);
			      break;
		default:
				WAPI_TRACE(WAPI_ERR, "%s: Unknown TID \n", __FUNCTION__);
				break;
	...
	if( IS_MCAST(pRA) ){
		...
		if (bFindMatchPeer){
			...
			if (pWapiSta->wapiUsk.keyId <= 1){
				...
				mem_cpy(pWapiExt->PN, pWapiSta->lastTxUnicastPN, 16);
				if (bPNOverflow){
	...
	)
{
	...
	mem_cpy(pframe+pattrib->hdrlen+pattrib->iv_len+pattrib->pktlen-pattrib->icv_len,
			(u8 *)MicBuffer,
		...
		if (pWapiSta->wapiUsk.keyId == KeyIdx && pWapiSta->wapiUsk.bSet){
			...
			if(precv_hdr->bWapiCheckPNInDecrypt){
				...
				if(bQosData){
					...
					mem_cpy(pWapiSta->lastRxUnicastPN, pRecvPN, 16);
				}
			}else{
				mem_cpy(precv_hdr->WapiTempPN,pRecvPN,16);
			}
	...
	if (0 == memcmp(MicBuffer, pRecvMic, padapter->wapiInfo.extra_postfix_len)){
		...
		if (bUseUpdatedKey){
			...
			if ( IS_MCAST(pRA) ){
				...
				mem_cpy(pWapiSta->wapiMsk.dataKey, pWapiSta->wapiMskUpdate.dataKey, 16);
				mem_cpy(pWapiSta->wapiMsk.micKey, pWapiSta->wapiMskUpdate.micKey, 16);
