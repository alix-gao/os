...
u32 mp_join(PADAPTER padapter,u8 mode)
{
	...
	if( mode==WIFI_FW_ADHOC_STATE ){
		bssid.Ssid.SsidLength = str_len("mp_pseudo_adhoc");
		...
		bssid.Ssid.SsidLength = str_len("mp_pseudo_STATION");
...
u32 mp_query_psd(PADAPTER pAdapter, u8 *data)
{
	...
	if (str_len(data) == 0) { //default value
		psd_pts = 128;
		psd_start = 64;
		psd_stop = 128;
	} else {
...
u8 rtw_mpRateParseFunc(PADAPTER pAdapter, u8 *targetStr)
{
	...
	for(i=0;i<=83;i++){
		if(str_cmp(targetStr, rateindex_Array[i]) == 0){
			DBG_871X("%s , index = %d \n",__func__ ,i);
			return i;
		}
	}
