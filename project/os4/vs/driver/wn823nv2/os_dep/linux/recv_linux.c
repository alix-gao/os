...
int rtw_os_alloc_recvframe(_adapter *padapter, union recv_frame *precvframe, u8 *pdata, _pkt *pskb)
{
	...
	if(pkt_copy)
	{
// alic		pkt_copy->dev = padapter->pnetdev;
...
void rtw_os_recv_indicate_pkt(_adapter *padapter, _pkt *pkt, struct rx_pkt_attrib *pattrib)
{
	...
	if (pkt) {
		if(check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE)
		{
			...
			if (_rtw_memcmp(pattrib->dst, adapter_mac_addr(padapter), ETH_ALEN) == _FALSE)
			{
				...
				if(psta)
				{
					...
					// alic pkt->dev = pnetdev;
#if (LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,35))
					skb_set_queue_mapping(pkt, rtw_recv_select_queue(pkt));
#endif //LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,35)

					_rtw_xmit_entry(pkt, 0);// alic pnetdev);
		...
		// alic pkt->protocol = eth_type_trans(pkt, padapter->pnetdev);
		// alic pkt->dev = padapter->pnetdev;
...
#else
        process_packet(padapter->dev_id, pkt->data, pkt->len);
        rtw_skb_free(pkt);
#endif
	}
}
...
#endif
// alic	union iwreq_data wrqu;
// alic	struct iw_michaelmicfailure    ev;
	struct mlme_priv*              pmlmepriv  = &padapter->mlmepriv;
	...
	else
	{
		...
		if( cur_time - psecuritypriv->last_mic_err_time < 60*OS_HZ )
		{
...
// alio	rtw_netif_rx(padapter->pnetdev, skb);
...
os_ret OS_CALLBACK _rtw_reordering_ctrl_timeout_handler(os_u32 FunctionContext);
os_ret OS_CALLBACK _rtw_reordering_ctrl_timeout_handler(os_u32 FunctionContext)
{
...
void rtw_init_recv_timer(struct recv_reorder_ctrl *preorder_ctrl)
{
	...
	_init_timer(&(preorder_ctrl->reordering_ctrl_timer), 0, _rtw_reordering_ctrl_timeout_handler, preorder_ctrl);
