...
)
{
	...
	mem_cpy(&(pRA_Table->per_rate_retrylimit_20M[0]), &(per_rate_retrylimit_table_20M[0]), ODM_NUM_RATE_IDX);
	mem_cpy(&(pRA_Table->per_rate_retrylimit_40M[0]), &(per_rate_retrylimit_table_40M[0]), ODM_NUM_RATE_IDX);
		...
		for (i = 0; i < NUM_STA; i++) {
			...
			while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
				...
				if (_rtw_memcmp(psta->hwaddr, bcast_addr, ETH_ALEN) ||
					_rtw_memcmp(psta->hwaddr, adapter_mac_addr(Adapter), ETH_ALEN))
					continue;
...
)
{
	...
	case (ODM_WM_A|ODM_WM_N5G): {
		if (pDM_Odm->RFType == ODM_1T2R || pDM_Odm->RFType == ODM_1T1R) {
			...
			if (rssi_level == DM_RATR_STA_HIGH)
				rate_bitmap = 0x0000000f0f0f0000LL;
			else if (rssi_level == DM_RATR_STA_MIDDLE)
				rate_bitmap = 0x0000000fcfcfe000LL;
			else {
				if (*(pDM_Odm->pBandWidth) == ODM_BW40M)
					rate_bitmap = 0x0000000ffffff015LL;
				else
					rate_bitmap = 0x0000000ffffff005LL;
			}
		}
	}
	break;
	...
	case (ODM_WM_AC|ODM_WM_A):
		...
		if (pDM_Odm->RFType == ODM_1T2R || pDM_Odm->RFType == ODM_1T1R) {
			...
			if (rssi_level == 1)				/* add by Gary for ac-series */
				rate_bitmap = 0x000003f8fe3f8000LL;       /* VHT 3SS MCS3~9 */
			else if (rssi_level == 2)
				rate_bitmap = 0x000003fffffff000LL;       /* VHT3SS MCS0~9 */
			else
				rate_bitmap = 0x000003fffffff010LL;       /* All */
		}
		break;
	...
	default:
		if (pDM_Odm->RFType == ODM_1T2R || pDM_Odm->RFType == ODM_1T1R)
			...
			rate_bitmap = 0x0000003fffffffffLL;
		break;
