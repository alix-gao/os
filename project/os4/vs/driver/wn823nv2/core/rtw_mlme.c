...
_func_enter_;
	...
	if(str_cmp(dst->Ssid.Ssid, DBG_RX_SIGNAL_DISPLAY_SSID_MONITORED) == 0) {
...
_func_enter_;
	...
	RT_TRACE(_module_rtl871x_mlme_c_,_drv_info_,("\n rtw_joinbss_event_callback !! _enter_critical %d\n", pnetwork->join_res));
...
flog("%s %s\n", competitor->network.Ssid.Ssid, mlme->assoc_ssid.Ssid);
...
flog("rtw_is_desired_network start\n");
	if(rtw_is_desired_network(adapter, competitor)  == _FALSE)
		goto exit;
flog("rtw_is_desired_network end, %d\n", rtw_to_roam(adapter));
print("check %d %x %x\n", rtw_to_roam(adapter), mlme, adapter);
#ifdef  CONFIG_LAYER2_ROAMING
	if(rtw_to_roam(adapter) > 0) {
		if(	rtw_get_passing_time_ms((u32)competitor->last_scanned) >= mlme->roam_scanr_exp_ms
			|| is_same_ess(&competitor->network, &mlme->cur_network.network) == _FALSE
		) {
		    flog("%d, %d %d\n", rtw_get_passing_time_ms((u32)competitor->last_scanned), is_same_ess(&competitor->network, &mlme->cur_network.network),
		         mlme->cur_network.network.Ssid.SsidLength);
			goto exit;
        }
	}
#endif
...
print("rtw_select_and_join_from_scanned_queue %d %x\n", rtw_to_roam(adapter), adapter);
	_enter_critical_bh(&(pmlmepriv->scanned_queue.lock), &irqL);
	...
	while (!rtw_end_of_queue_search(phead, pmlmepriv->pscanned)) {
		...
		if (1)
			DBG_871X("%s("MAC_FMT", ch%d) rssi:%d\n"
				, pnetwork->network.Ssid.Ssid
				, MAC_ARG(pnetwork->network.MacAddress)
				, pnetwork->network.Configuration.DSConfig
				, (int)pnetwork->network.Rssi);
	...
	if(candidate == NULL) {
		...
		DBG_871X("%s: candidate: %s("MAC_FMT", ch:%d)\n", __FUNCTION__,
			candidate->network.Ssid.Ssid, MAC_ARG(candidate->network.MacAddress),
			candidate->network.Configuration.DSConfig);
		goto candidate_exist;
	}
...
candidate_exist:
    ...
    do {
        os_u8 sec_ies[0x100];
        init_net_dev(adapter, candidate->network.Ssid.Ssid, candidate->network.Ssid.SsidLength, candidate->network.MacAddress);
        if (1) {
            ret = gen_ie_by_parse_scan(sec_ies, sizeof(sec_ies), candidate->network.IEs, candidate->network.IELength);
        } else {
            os_u8 sec_ie[] = { /* fix ies */ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x30, 0x14, 0x01, 0x00, 0x00, 0x0f, 0xac, 0x02,
                0x01, 0x00, 0x00, 0x0f, 0xac, 0x04, 0x01, 0x00,
                0x00, 0x0f, 0xac, 0x02, 0x00, 0x00 };
            ret = gen_ie_by_parse_scan(sec_ies, sizeof(sec_ies), sec_ie, sizeof(sec_ie));
        }
        if (OS_SUCC == ret) {
            update_sec_info(adapter, sec_ies, 2 + sec_ies[1]);
        }
    } while (0);
	set_fwstate(pmlmepriv, _FW_UNDER_LINKING);
...
void rtw_build_wmm_ie_ht(_adapter *padapter, u8 *out_ie, u32 *pout_len)
{
	...
	if(padapter->mlmepriv.qospriv.qos_option == 0)
	{
		...
		pframe = rtw_set_ie(out_ie+out_len, _VENDOR_SPECIFIC_IE_,
							_WMM_IE_Length_, WMM_IE, (u32 *) pout_len);
...
unsigned int rtw_restructure_ht_ie(_adapter *padapter, u8 *in_ie, u8 *out_ie, uint in_len, u32 *pout_len, u8 channel)
{
	u32 ielen, out_len;
	...
	*/
	...
	pframe = rtw_set_ie(out_ie+out_len, _HT_CAPABILITY_IE_,
						sizeof(struct rtw_ieee80211_ht_cap), (unsigned char*)&ht_capie, (u32 *) pout_len);

	phtpriv->ht_option = _TRUE;

	if(in_ie!=NULL)
	{
		p = rtw_get_ie(in_ie, _HT_ADD_INFO_IE_, &ielen, (u32) in_len);
		if(p && (ielen==sizeof(struct ieee80211_ht_addt_info)))
		{
			out_len = *pout_len;
			pframe = rtw_set_ie(out_ie+out_len, _HT_ADD_INFO_IE_, ielen, p+2 , (u32 *) pout_len);
		}
	}
...
void rtw_append_exented_cap(_adapter *padapter, u8 *out_ie, u32 *pout_len)
{
	struct mlme_priv 	*pmlmepriv = &padapter->mlmepriv;
	struct ht_priv		*phtpriv = &pmlmepriv->htpriv;
	...
	*/
	if (_FALSE == _rtw_memcmp(cap_content, null_content, 8))
		pframe = rtw_set_ie(out_ie + *pout_len, EID_EXTCapability, 8, cap_content , (u32 *)pout_len);
}
#endif
