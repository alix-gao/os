...
#ifdef CONFIG_C2H_WK
// alic static void c2h_wk_callback(_workitem *work);
#endif
...
#ifdef CONFIG_C2H_WK
// alic	_init_workitem(&pevtpriv->c2h_wk, c2h_wk_callback, NULL);
...
#ifdef CONFIG_C2H_WK
// alio	_cancel_workitem_sync(&pevtpriv->c2h_wk);
...
// alic		|| ATOMIC_READ(&(pcmdpriv->cmdthd_running)) == _FALSE	//com_thread not running
	)
	{
		DBG_871X("%s: drop cmdcode:%d, hw_init_completed:%d, cmdthd_running:%d\n", __FUNCTION__,
			cmd_obj->cmdcode,
...
post_process:
		...
		if(pcmd->cmdcode < (sizeof(rtw_cmd_callback) /sizeof(struct _cmd_callback)))
		{
			...
			{
				RT_TRACE(_module_rtl871x_cmd_c_,_drv_info_,("mlme_cmd_hdl(): pcmd_callback=0x%x, cmdcode=0x%x\n", pcmd_callback, pcmd->cmdcode));
