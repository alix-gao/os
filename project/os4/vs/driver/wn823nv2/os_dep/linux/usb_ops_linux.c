...
int usbctrl_vendorreq(struct intf_hdl *pintfhdl, u8 request, u16 value, u16 index, void *pdata, u16 len, u8 requesttype)
{
	...
	while(++vendorreq_times<= MAX_USBCTRL_VENDORREQ_TIMES)
	{
		...
		if (requesttype == 0x01)
		{
			// alic pipe = usb_rcvctrlpipe(udev, 0);//read_in
			reqtype =  REALTEK_USB_VENQT_READ;
		}
		else
		{
			// alic pipe = usb_sndctrlpipe(udev, 0);//write_out
			reqtype =  REALTEK_USB_VENQT_WRITE;
			_rtw_memcpy( pIo_buf, pdata, len);
		}
//flog("=====%d, %d\n", reqtype, value);
		// alic status = rtw_usb_control_msg(udev, pipe, request, reqtype, value, index, pIo_buf, len, RTW_USB_CONTROL_MSG_TIMEOUT);
        do {
            struct usb_setup_data cmd;
            cmd.bmRequestType = reqtype;
            cmd.bRequest = request;
            cmd.wValue = value;
            cmd.wIndex = index;
            cmd.wLength = len;
            if (1 == requesttype) {
                status = recv_usb_control_data(udev, &cmd, pIo_buf, len);
            } else if (0 == requesttype) {
                status = send_usb_control_data(udev, &cmd, pIo_buf, len);
            } else {
                status = OS_FAIL;
            }
        } while (0);
        ...
        if (OS_SUCC == status)
        {
			...
			DBG_8192C("reg 0x%x, usb %s %d fail, status:%d value=0x%x, vendorreq_times:%d\n"
...
release_mutex:
	...
	#endif
		if (0) { //(requesttype == 0x01) {
		    os_u32 *show;
		    show = (os_u32 *) pdata;
		    flog("control msg\n");
			for (status = 0; status < 0x10; status++) {
			    flog("%d ", show[status]);
			} flog("\n");
		}
exit:
...
unsigned int ffaddr2pipehdl(struct dvobj_priv *pdvobj, u32 addr)
{
	...
	if (addr == RECV_BULK_IN_ADDR) {
		// alic pipe=usb_rcvbulkpipe(pusbd, pdvobj->RtInPipe[0]);
		pipe = pdvobj->RtInPipe[0];

	} else if (addr == RECV_INT_IN_ADDR) {
		// alic pipe=usb_rcvintpipe(pusbd, pdvobj->RtInPipe[1]);
		pipe = pdvobj->RtInPipe[1];

	} else if (addr < HW_QUEUE_ENTRY) {
		ep_num = pdvobj->Queue2Pipe[addr];
		// alic pipe = usb_sndbulkpipe(pusbd, ep_num);
		pipe = ep_num;
	}
...
static void usb_write_port_complete(struct xmit_buf *pxmitbuf, int status)
{
	_irqL irqL;
	int i;
...
check_completion:
	...
	status ? RTW_SCTX_DONE_WRITE_PORT_ERR : RTW_SCTX_DONE_SUCCESS);
	...
	{
// alic		tasklet_hi_schedule(&pxmitpriv->xmit_tasklet);
	}
...
// alic	purb	= pxmitbuf->pxmit_urb[0];
    ...
    flog("send %d\n", system_tick());
    status = send_usb_bulk_data(pusbd, pipe, pxmitframe->buf_addr, cnt);

    usb_write_port_complete(pxmitbuf, status);
    flog("send end %d\n", system_tick());
#endif
	if (!status) {
		#ifdef DBG_CONFIG_ERROR_DETECT
...
#if 0 // alic mod for recv mechnism
void usb_recv_tasklet(void *priv)
{
	_pkt			*pskb;
	_adapter		*padapter = (_adapter*)priv;
...
void usb_recv_tasklet(void *priv)
{
    ...
    print("start recv......\n");
    flog("usb_recv_tasklet\n");
    // active first
    if (NULL != (precvbuf = rtw_dequeue_recvbuf(&precvpriv->recv_buf_pending_queue))) {
		precvbuf->pskb = NULL;
		rtw_read_port(padapter, precvpriv->ff_hwaddr, 0, (unsigned char *)precvbuf);
	}
    flog("usb_recv_tasklet start...\n");
	while (NULL != (pskb = skb_dequeue(&precvpriv->rx_skb_queue))) {
		if ((padapter->bDriverStopped == _TRUE)||(padapter->bSurpriseRemoved== _TRUE)) {
			DBG_8192C("recv_tasklet => bDriverStopped or bSurpriseRemoved \n");
			#ifdef CONFIG_PREALLOC_RX_SKB_BUFFER
			if (rtw_free_skb_premem(pskb) != 0)
			#endif //CONFIG_PREALLOC_RX_SKB_BUFFER
#if 0 // alic mod rx mechnism
				rtw_skb_free(pskb);
#else
				skb_queue_head(&precvpriv->rx_skb_queue, precvbuf->pskb);
#endif
			break;
		}

		recvbuf2recvframe(padapter, pskb);
//print("t%d", system_tick());
		skb_reset_tail_pointer(pskb);
		pskb->len = 0;
...
#endif
void usb_read_port_complete(struct recv_buf	*precvbuf, int status)
{
	_adapter 			*padapter =(_adapter *)precvbuf->adapter;
	struct recv_priv	*precvpriv = &padapter->recvpriv;
	...
	if(status==0)//SUCCESS
	{
		...
		precvbuf->transfer_len = precvbuf->alloc_sz;
		skb_put(precvbuf->pskb, precvbuf->alloc_sz);
#if 0 // alic mod rx mechnism
        skb_queue_tail(&precvpriv->rx_skb_queue, precvbuf->pskb);
#else
        skb_queue_head(&precvpriv->rx_skb_queue, precvbuf->pskb);
	...
	else
	{
		...
		if(rtw_inc_and_chk_continual_io_error(adapter_to_dvobj(padapter)) == _TRUE ){
			// alio padapter->bSurpriseRemoved = _TRUE;
		}
...
recv_buf_hook:
	...
	/* translate DMA FIFO addr to pipehandle */
	pipe = ffaddr2pipehdl(pdvobj, addr);
	mem_set(precvbuf->pbuf, 0, MAX_RECVBUF_SZ);
	err = recv_usb_bulk_data(pusbd, pipe, precvbuf->pbuf, MAX_RECVBUF_SZ);
    if (err != OS_SUCC) {
    	DBG_871X("cannot submit rx in-token(err = 0x%x),urb_status = %d\n", err, 0);
    	usb_read_port_complete(precvbuf, err);
    	ret = _FAIL;
    	goto exit;
    }
    do {
