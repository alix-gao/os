...
static int wl_cfgvendor_rtt_set_config(struct wiphy *wiphy, struct wireless_dev *wdev,
	...
	mem_set(&rtt_param, 0, sizeof(rtt_param));
			...
			nla_for_each_nested(iter1, iter, rem1) {
				nla_for_each_nested(iter2, iter1, rem2) {
					...
					case RTT_ATTRIBUTE_TARGET_MAC:
						mem_cpy(&rtt_target->addr, nla_data(iter2), ETHER_ADDR_LEN);
						break;
					...
					case RTT_ATTRIBUTE_TARGET_CHAN:
						mem_cpy(&rtt_target->channel, nla_data(iter2),
					...
					const void *data, int len)
{
	...
	nla_for_each_attr(iter, data, len, rem) {
		...
		case RTT_ATTRIBUTE_TARGET_MAC:
			if (mac_addr)
				mem_cpy(mac_addr++, nla_data(iter), ETHER_ADDR_LEN);
			else {
	...
	struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (strncmp((char *)data, BRCM_VENDOR_SCMD_CAPA, str_len(BRCM_VENDOR_SCMD_CAPA)) == 0) {
		...
		data_len = str_len(cfg->ioctl_buf);
